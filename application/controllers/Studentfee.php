...
class Studentfee extends Admin_Controller {
    ...
    function parentSearch(){
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            ...
            if (isset($search)) {
                $resultlist = $this->student_model->searchStudentByParentName($search_text);
    ...
    function addByParentfee(){
        ...
        if(count($studentIds)>0){
           // $dataInvoices = [""];
            $dataInvoices = [
                "invoice_amount"=> array_sum($amount),
                "parent_id"=>$parent_id,
                "status"=>"active"
            ];
	...
	$data["previous_session_balance_tax"] = $this->setting_model->getPreviousSessionBalanceTax();
        ...
        $data["invoice_number"] = $lastInsertId;
    ...
    function addstudentfee() {
        ...
        */
            ...
            $dataInvoices = [
                "invoice_amount"=> floatval($this->input->post('amount')),
                "parent_id"=>$parent_id,
                "status"=>"active"
            ];
            $lastInsertId = $this->invoices_model->add_invoices($dataInvoices);
