...
//die;
    ...
    public function searchStudentByParentName($searchterm, $carray = null) {
        ...
        $this->db->where('students.is_active', 'yes');
        $this->db->group_start();
        $this->db->where('students.guardian_name', $searchterm);
        $this->db->or_where('students.father_name', $searchterm);
         $this->db->or_where('students.guardian_id', $searchterm);
        $this->db->or_where('students.mother_name', $searchterm);
...
//die;
    ...
    function getStudentDetail($userId){
        $this->db->select('parent_id, father_name, guardian_id ,father_phone, father_occupation, count(*) as children')->from('students');
        $this->db->where('students.parent_id', $userId);
        $query = $this->db->get();
        return $query->result();
    }
