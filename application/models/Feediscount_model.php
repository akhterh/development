...
class Feediscount_model extends CI_Model {
    ...
    public function getPreviousDiscountIds($admission_no){
    ...
    public function addBulkImportDiscounts($dataArray){
        $count = 0;
        foreach ($dataArray as $dA){
           $count +=  $this->insertStudentDiscount($dA["discount_ids"], $dA["student_admission_no"]);
        }
    ...
    public function insertStudentDiscount($discountIds, $admission_no){
        $data["student_admission_no"] = $admission_no;
        $data["discount_ids"] = $discountIds;

        $this->db->where("student_admission_no", $admission_no);

        $temp = $this->db->get("student_indiviual_discount");

        if($temp->num_rows() > 0){
            $this->db->where('student_admission_no',$admission_no);
            $this->db->update('student_indiviual_discount',$data);
        }else{
            $this->db->insert("student_indiviual_discount", $data);
        }
