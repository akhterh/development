...
class Studentsession_model extends CI_Model {
    ...
    public function getStudentByClass($student_id) {
     $this->db->select('student_session.session_id, student_session.class_id,classes.class
            ')->from('student_session');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        // $this->db->join('sections', 'sections.id = student_session.section_id');
        // $this->db->join('students', 'students.id = student_session.student_id');
    ...
    public function getTotalStudentBySession() {
     ...
     public function updateClasses($dataArray){
        $rowCount = 0;
        
        foreach($dataArray as $dArray){
            $query = "UPDATE `student_session` SET `class_id` = ".$dArray['class_id']. ', `section_id` = '. $dArray['section_id'].' WHERE `student_session`.`student_id` = '.$dArray['student_id'];
            $this->db->query($query);
